...
public class CollateralMessageAdapter extends RecyclerView.Adapter<CollateralMessageAdapter.ViewHolder> {
    ...
    private List<Collateral> data = new ArrayList<>();
    private List<Collateral> list = new ArrayList<>();


    public CollateralMessageAdapter(Context context, List<Collateral> data) {
    ...
    @NonNull
    @Override
    public CollateralMessageAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(context).inflate(R.layout.collateral_meesage_row_design, parent, false);
        return new ViewHolder(v);
    }

    public void setAgentsList(List<Collateral> list) {
        data.addAll(list);
        notifyDataSetChanged();
    }

    @Override
    public void onBindViewHolder(@NonNull final CollateralMessageAdapter.ViewHolder viewHolder, final int position) {
        try {
            if (data != null && position < data.size()) {


                final Collateral collateral = data.get(position);
                if (collateral != null) {
                    viewHolder.message_tv.setText(Helper.getInstance().capitalize(collateral.getTitle()));
                }
                viewHolder.itemView.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        if (context instanceof ChatActivity) {
                            ((ChatActivity) context).sendCollateral(ChatActivity.s3Url + collateral.getPath(), collateral.getTitle(), "");
                        }
                    }
                });
    ...
    private Collateral getItem(int position) {
        try {
    ...
    public void resetList(List<Collateral> collaterals) {
        ...
        List<Collateral> temp = new ArrayList<>();
        temp.addAll(collaterals);
    ...
    private void addItemToSearchList(Collateral collateral) {
        try{
        if (data != null) {
            if (data.size() == 0) {
                ...
                if (data.size() > 0) {
                    ...
                    for (int i = 0; i < data.size(); i++) {
                        if (data.get(i).getAssetId() != null && collateral.getAssetId() != null && data.get(i).getAssetId().equals(collateral.getAssetId())) {
                            isPresent = true;
                            break;
                        }
                    }
